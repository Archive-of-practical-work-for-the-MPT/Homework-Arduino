...
- Замените название WiFi и пароль на свой
- Загрузите скетч и откройте Serial Monitor
- В Serial Monitor отобразится адрес, который можно открыть на любом устройстве, который подключен к WiiFi, что вы указали
- Переключайте ползунки на локальном сайте, лампочки будут менять яркость
